...
interface Comment {
  ...
  } | null; // Kullanıcı silinmişse null olabilir
}
          ...
          <div className="space-y-4 mb-8 mt-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              comments.map((comment) => {
                // Kullanıcı adını ve resmini güvenli şekilde alalım
                const displayName = comment.user?.username || comment.user?.firstName || "Bilinmeyen Oyuncu";
                const displayImage = comment.user?.imageUrl || "/file.svg";
                  ...
                  <div key={comment.id} className="flex gap-4 p-4 bg-[#0f192b] rounded-xl border border-white/5 hover:border-cyan-500/20 transition-all group/comment">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-white text-sm font-brand tracking-wide">{displayName}</span>
