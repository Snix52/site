...
export async function POST(req: Request) {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: realUserId },
      select: { id: true, isBanned: true, currentPoints: true, ownedFrames: true, username: true },
    });
    ...
    const updatedUser = await prisma.$transaction(async (tx) => {
      const freshUser = await tx.user.findUnique({
        where: { id: realUserId },
        select: { currentPoints: true, ownedFrames: true, isBanned: true },
      });
